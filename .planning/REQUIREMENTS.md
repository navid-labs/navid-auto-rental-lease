...
- [ ] **VEHI-01**: Dealer can register vehicle with details (make, model, year, mileage, color, price)
- [ ] **VEHI-02**: Dealer can upload multiple vehicle images
- [ ] **VEHI-03**: Dealer can edit/delete own vehicle listings
- [ ] **VEHI-04**: Admin can register/edit/delete vehicles (self-operated inventory)
- [ ] **VEHI-05**: Vehicle status transitions (available → reserved → rented → maintenance)
- [ ] **VEHI-06**: License plate auto-lookup via API with manual input fallback
...
- [ ] **PRIC-01**: Monthly rental/lease payment calculator based on contract terms
- [ ] **PRIC-02**: Residual value estimation from admin-configurable lookup table (make/model/year → %)

### Contract

- [ ] **CONT-01**: Multi-step contract application form (vehicle → terms → eKYC → review → submit)
- [ ] **CONT-02**: Mock eKYC flow with ID verification UI (real API integration in v2)
- [ ] **CONT-03**: Contract PDF auto-generation with all contract details
- [ ] **CONT-04**: Contract status tracking on customer my page
- [ ] **CONT-05**: Real-time vehicle/contract status updates via Supabase Realtime
- [ ] **CONT-06**: Contract state machine with explicit transitions (draft → pending_ekyc → pending_approval → approved → active → completed)
- [ ] **CONT-07**: Admin approval step after contract submission
