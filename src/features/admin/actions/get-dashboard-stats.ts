...
export async function getDashboardStats(): Promise<DashboardStats> {
  ...
  function formatVehicleName(vehicle: {
    ...
    const brand = vehicle.trim.generation.carModel.brand.nameKo ?? vehicle.trim.generation.carModel.brand.name
    const model = vehicle.trim.generation.carModel.nameKo ?? vehicle.trim.generation.carModel.name
    return `${brand} ${model}`
  }
