...
const { mockPrisma, mockStorageUpload, mockStorageRemove, mockGetPublicUrl } = vi.hoisted(() => {
...
import { uploadImageMutation, deleteImageMutation, reorderImagesMutation } from './images'
...
describe('uploadImageMutation', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('rejects CUSTOMER role', async () => {
    const result = await uploadImageMutation('v1', createMockFormData(), customerUser)
  ...
  it('rejects dealer uploading to another dealers vehicle', async () => {
    ...
    const result = await uploadImageMutation('v1', createMockFormData(), dealerUser)
  ...
  it('sets isPrimary false for non-first images', async () => {
    ...
    const result = await uploadImageMutation('v1', createMockFormData(), adminUser)
...
describe('deleteImageMutation', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('rejects CUSTOMER role', async () => {
    const result = await deleteImageMutation('img-1', customerUser)
  ...
  it('rejects dealer deleting other dealers image', async () => {
    ...
    const result = await deleteImageMutation('img-1', dealerUser)
...
describe('reorderImagesMutation', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('rejects CUSTOMER role', async () => {
    const result = await reorderImagesMutation('v1', ['img-1', 'img-2'], customerUser)
  ...
  it('updates order and isPrimary via transaction', async () => {
    ...
    const result = await reorderImagesMutation('v1', ['img-2', 'img-1', 'img-3'], dealerUser)
